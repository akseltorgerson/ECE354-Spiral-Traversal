...
void get_dimensions(FILE *fp, int *size) {
    char *buffer = NULL;
    size_t len = 0;
    if (getline(&buffer, &len, fp) == -1) { // if get line returns -1 there is an error
...
int main(int argc, char *argv[]) {
    ...
    int **spiralptr;
    spiralptr = malloc(sizeof(int*) * size); // need to check lib fucntions to make sure they return the correct values
    if(spiralptr == NULL){
    	printf("Malloc failed when allocating data to the array of array");
	exit(1);
    }
    for(int i = 0; i < size; i++){
        *(spiralptr + i) = malloc(sizeof(int) * size); // need to check lib fucntions to make sure they return the correct values
	if((spiralptr + i) == NULL){
		printf("Malloc failed when allocating data to array ");
		printf("%d\n", i);
	}
    }
    ...
    Square* spiralBoard; // declare a Square
    spiralBoard = malloc(sizeof(Square)); // allocate memory to it
    spiralBoard->size = size; // set the size equal the size of the dimensions of the array
    spiralBoard->array = spiralptr; // set the pointer to point to the array
    ...
    free(spiralptr); // frees memory allocated to the spiral board pointer
    spiralptr =  NULL; // null the pointer to avoid dangling pointer

    free(spiralBoard); // frees memory allocated to the square data structure
    spiralBoard = NULL; // null the pointer to avoid dangling pointer
